...
def phagedynamics(y):
    ...
    if y[1] > 0:        absorption0 = param['absorption']
    else:               absorption0 = 0
    if y[2] > 0:        absorption1 = param['absorption']
    else:               absorption1 = 0
    if y[1] + y[2] > 0: absorptionp = (y[1] * absorption0 + y[2] * absorption1)/(y[1] + y[2])
    else:               absorptionp = 0
    
    
    return np.array([   growthrate * y[0] - absorption0 * y[4],                      # 0  susceptible bacteria
                        growthrate * y[1] - absorption1 * y[4] + recovery * y[3],    # 1  resistant bacteria
                        absorption0 * y[4] - y[2]/latencytime,                       # 2  infected susceptible bacteria
                        absorption1 * y[4] - y[3]/latencytime - recovery * y[3],     # 3  infected resistant bacteria
                        burstsize/latencytime * (y[2] + [3]) - absorptionp*y[4],     # 4  phages
...
def main():
    ...
    parser.add_argument("-n","--param_absorption",type=float,default=0.023)
    parser.add_argument("-y","--param_nutrientspercell",type=float,default=2e-10) # also in dilutions of original LB medium
    parser.add_argument("-r","--param_resitant_reduction_rate",type=float,default=1.5)
